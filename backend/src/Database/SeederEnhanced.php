...
class SeederEnhanced
{
    ...
    private function calculateValueMultiplier(float $temp, float $atmo, float $water, float $grav, float $rad): float
    {
        ...
        if ($rad > 0.8) {
            $base += 0.3; // High radiation = dangerous but valuable
        }
        if ($temp > 100 || $temp < -50) {
            $base += 0.2; // Extreme temperatures
        }
        if ($grav > 1.5 || $grav < 0.4) {
            $base += 0.1; // Unusual gravity
        }
    ...
    private function calculateRarity(string $name): int
    {
        ...
        if (in_array($name, $rare)) {
            return 1;
        }
        if (in_array($name, $uncommon)) {
            return 2;
        }
        return 3; // Common
    }
    ...
    private function determinePlanetPreferences(array $species): array
    {
        ...
        if ($avgTemp > 60) {
            $preferences[] = 'Volcanic';
        }
        if ($avgTemp < -10) {
            $preferences[] = 'Ice World';
        }
        if (($species['waterRange'][1] ?? 0.5) > 0.7) {
            $preferences[] = 'Ocean World';
        }
        if (($species['atmoRange'][1] ?? 1.0) < 0.3) {
            $preferences[] = 'Rocky';
        }
    ...
    {
        if (strpos($name, 'Crystal') !== false) {
            return 'crystalline';
        }
        if (strpos($name, 'Aquatic') !== false) {
            return 'smooth';
        }
    ...
    private function determineEffectType(array $tool): string
    {
        ...
        if (isset($effect['temperature'])) {
            return 'temperature_modifier';
        }
        if (isset($effect['atmosphere'])) {
            return 'atmosphere_modifier';
        }
        if (isset($effect['water'])) {
            return 'water_modifier';
        }
        if (isset($effect['gravity'])) {
            return 'gravity_modifier';
        }
        if (isset($effect['radiation'])) {
            return 'radiation_modifier';
        }
    ...
    private function generateUnlockRequirements(array $tool): array
    {
        ...
        $requirements = [];
        if ($tier >= 2) {
            $requirements['planets_visited'] = $tier * 3;
        }
        if ($tier >= 3) {
            $requirements['credits_earned'] = $tier * 10000;
        }
    ...
    {
        if (strpos($name, 'Kepler') !== false) {
            return 'exoplanet';
        }
        if (strpos($name, 'TRAPPIST') !== false) {
            return 'system';
        }
        return 'general';
    }
