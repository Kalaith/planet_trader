...
const PlanetPurchaseModal: React.FC = () => {
  const { planetModalOpen, planetOptions, closePlanetModal, purchasePlanet } =
    useGameContext();
        ...
        <div className="p-3 sm:p-6">
          ...
          <div className="space-y-3 sm:space-y-4">
            {planetOptions.map((opt) => (
              <div
                key={opt.id || opt.name}
                className="bg-gray-700 border border-gray-600 rounded-lg p-3 sm:p-4"
              >
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
                  <div className="flex-1 w-full sm:w-auto">
                    <div className="font-bold text-white text-base sm:text-lg mb-2">
                      {opt.name}
                    </div>
                  ...
                  <div className="text-center w-full sm:w-auto sm:ml-4">
                    <div className="text-green-400 font-bold text-lg mb-2">
                      {opt.purchasePrice.toLocaleString()}₵
                    </div>
                    <button
